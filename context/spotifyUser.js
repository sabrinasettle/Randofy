...
const withSpotify = Component => {
  class WithSpotify extends Component {
    ...
    checkTime = async () => {
      ...
      {
        return await axios.get(`https://randofy-backend.herokuapp.com/token/refresh`, {params: {refresh_token: this.state.auth.refresh_token}})
        .then(response => {
          ...
          response.data.expires_at = new Date().setSeconds(response.data.created_at.getSeconds() + seconds);
    ...
    getUser = async () => {
      ...
      return await axios.get(`https://api.spotify.com/v1/me`, {headers: {'Authorization': `Bearer ${this.state.auth.access_token}`}})
      .then(response => {
    ...
    getPlaylist = async (playlistId) => {
      await this.checkTime();
      return await axios.get(`https://api.spotify.com/v1/playlists/${playlistId}`, {headers: {'Authorization': `Bearer ${this.state.auth.access_token}`}})
      .then(response => {
        const playlist = {id: response.data.id, tracks: response.data.tracks.items, link: response.data.href}
        this.setState({
          playlist: playlist
        })
    ...
    createPlaylist = async () => {
      ...
      {
        'name': 'Randofy',
        'description': 'Your random songs from Randofy https://randofy.vercel.app',
        'public': true,
      },
      {
    ...
    removeFromPlaylist = async (songId) => {
      ...
      if (!this.state.songIds.includes(songId)){
        ...
        const songUri = 'spotify:track:' + songId;
        return await axios.delete(`https://api.spotify.com/v1/playlists/${this.state.playlist.id}/tracks`, 
        {
          headers: {
            'Authorization': `Bearer ${this.state.auth.access_token}`,
            'Accept': 'application/json',
