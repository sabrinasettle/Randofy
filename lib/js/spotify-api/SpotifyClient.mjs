...
class SpotifyClient {
  ...
  //array of ids
  //query is the array
  async getTracksAudioFeatures(ids, options) {
    return this.createSpotifyRequest(`audio-features?ids=${ids}`, options);
  }
  ...
  async getReccomendations(query, options) {
    ...
    Object.keys(options).map((key) => {
      if (key == "numberOfSongs") {
        console.log("key", key, options[key]);
        params.set("limit", options[key]);
      } else {
        params.set(`min_${key}`, JSON.stringify(options[key][0]));
        params.set(`max_${key}`, JSON.stringify(options[key][1]));
      }
    });
    //add Market
    const genereString = `${this.genres[0]},${this.genres[1]},${this.genres[2]},${this.genres[3]}`;
    params.set("seed_genres", genereString);
    console.log(`recommendations?` + params.toString());
    return this.createSpotifyRequest(`recommendations?` + params.toString(), {
      headers: { method: "GET" },
    });
  }
}
