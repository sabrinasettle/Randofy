...
class SpotifyClient {
  ...
  async getArtists(ids, options) {
    return this.createSpotifyRequest(`artists?ids=${ids.join(",")}`, options);
  }
  ...
  async getRecommendations(query, options) {
    ...
    if (!options.includes("seed_genres=")) {
      // const genereString = `${this.genres[10]},${this.genres[1]},${this.genres[2]},${this.genres[20]}`;
      const genresLen = getRandomInt(5);
      let genres = [];
      for (let i = 0; i <= genresLen; i++) {
        const genre = this.genres[getRandomInt(this.genres.length)];
        if (!genres.includes(genre)) {
          genres.push(genre);
        }
      }
      const genereString = genres.join(",");
      // console.log("string of genres", genereString);
