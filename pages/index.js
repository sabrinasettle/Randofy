  ...
  List = ({ spotifyUser }) => {
    ...
    let htmlList = (
      <ul className={liststyles.list}>
        ...
        {len > 1 ? (
          <h1 className={liststyles.count}>{len} songs generated</h1>
        ) : (
          <h1 className={liststyles.count}>{len} song generated</h1>
        )}
        {this.state.songList.map((dataObj, index) => {
          ...
          if (spotifyUser && spotifyUser.songIds) {
            !spotifyUser.songIds.includes(dataObj.track_id)
              ? (song = false)
              : (song = true);
          }
          ...
          return (
            <li className={liststyles.cards} key={dataObj.track_id}>
              <div className={liststyles.fbcard}>
                <div className={liststyles.index}>{i}</div>
                <div className={liststyles.span}>
                  ...
                  {dataObj.preview_url && (
                    <audio controls>
                      <source src={dataObj.preview_url}></source>
                    </audio>
                  )}
          ...
          {(spotifyUser) =>
            spotifyUser && spotifyUser.spotifyUser ? (
              ...
              <div>
                ...
                <MainButton updateList={this.updateList} />
                <div className={styles.sectionmain}>
                  {songList.length < 1 ? <p></p> : <this.List />}
                </div>
              </div>
            )
          }
