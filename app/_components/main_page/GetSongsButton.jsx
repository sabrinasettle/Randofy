...
export default function GetSongsButton({ isSmall }) {
  ...
  return (
    <button
      id="__get_songs"
      className={`flex-1 font-body w-full md:w-auto transition-all bg-gray-600 border border-transparent hover:border-gray-600 hover:bg-gray-700 rounded-sm duration-400 ease-in-out ${
        isSmall ? "text-heading-6" : "text-heading-5"
      } ${classString}`}
      onClick={musicContext.getSongs}
    >
      Get Songs
    </button>
  );
